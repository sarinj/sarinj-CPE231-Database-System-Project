...
$con = mysqli_connect($servername,$username,$password,'b1n_bitnance');
                    ...
                    if ((!empty($_POST['currency'])) && (!empty($_POST['amount']))) {
                        if ($_POST['currency'] != 'cash') {
                            ...
                            $available = mysqli_fetch_array(mysqli_query($con,"SELECT SUM(CASE when is_deposite = 1 THEN quantity ELSE -quantity END) AS balance
                                                FROM transaction_wallet WHERE wallet_id = $wallet_id AND item_id = $item_id AND is_offer = 0"));
                ...
                <table class="table table-hover wallet_table">
                    ...
                    <tbody><?php
                               ...
                               while ($coindata = mysqli_fetch_array($coinsdata2)){ 
                                ...
                                $in_offer = mysqli_fetch_array(mysqli_query($con,"SELECT SUM(CASE when is_deposite = 1 THEN quantity ELSE -quantity END) AS balance
                                                FROM transaction_wallet WHERE wallet_id = $wallet_id AND item_id = $item_id AND is_offer = 1"));
